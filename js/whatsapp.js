...
import { db, collection, addDoc, getDocs, query, where, orderBy, limit, writeBatch, serverTimestamp, doc, getDoc } from './firebaseConfig.js';
...
const serverStatus = {
    ...
    async restartSession() {
        try {
            console.log("Attempting session restart...");
            ...
            try {
                const checkResponse = await fetch(`${WAHA_CONFIG.baseUrl}/sessions/${WAHA_CONFIG.session}`, {
                    headers: WAHA_CONFIG.headers
                });
                ...
                if (checkResponse.status === 404) {
                    ...
                    console.log("Session exists, just restarting it");
                }
            } catch (e) {
                console.log("Session check failed, continuing anyway:", e);
            }
    ...
    async getQR() {
        try {
            ...
            if (qrCode) {
                ...
                console.error("Failed to retrieve QR code from WAHA API");
                return false;
            }
        } catch (error) {
        ...
        <div id="whatsappQrModal" class="modal" style="display: block; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.7);">
            <div class="modal-content" style="background-color: #fefefe; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 350px; border-radius: 8px; text-align: center;">
                ...
                <div style="margin: 20px 0;">
                    <img src="${imgSrc}" alt="WhatsApp QR Code" style="width: 256px; height: 256px; display: block; margin: 0 auto;">
                </div>
...
async function authenticateWithQR() {
    ...
    // If session is already connected, no need to authenticate
    if (serverStatus.sessionStatus === 'CONNECTED' || serverStatus.sessionStatus === 'WORKING') {
        console.log("Session is already authenticated and connected");
        return true;
    }
    
    // If session is in a bad state, restart it without deleting
    if (serverStatus.sessionStatus === 'SCAN_QR_CODE' || 
        ...
        // Use restart without deletion
        try {
            // Just stop and start the session without deleting
            await fetch(`${WAHA_CONFIG.baseUrl}/sessions/${WAHA_CONFIG.session}/stop`, {
                method: 'POST',
                headers: WAHA_CONFIG.headers
            });
            ...
            console.log("Session restart during authentication failed, continuing anyway:", e);
        }
    }
...
async function sendInvoiceNotification(user, invoice) {
    try {
        // Use formatMessage to get the template from Firestore
        const message = await formatMessage(user, 'INVOICE_NOTIFICATION', {
            ...invoice,
...
async function sendPaymentReminder(user, invoice) {
    // Use formatMessage to get the template from Firestore
    const message = await formatMessage(user, 'PAYMENT_REMINDER', {
        ...invoice,
        formattedId: invoice.formattedId || invoice.id,
        amount: invoice.amount,
        dueDate: invoice.dueDate,
...
async function sendPaymentConfirmation(user, invoice) {
    try {
        ...
        const message = await formatMessage(user, 'PAYMENT_CONFIRMATION', {
            ...invoice,
            formattedId: invoice.formattedId,
...
async function formatMessage(user, type, data) {
    // Map the type to the Firestore template type
    ...
    let template = '';
    try {
        const templateDoc = await getDoc(doc(db, "message_templates", templateType));
        if (templateDoc.exists()) {
            template = templateDoc.data().content;
        } else {
            // Fall back to hardcoded template
            template = MESSAGE_TEMPLATES[type] || '';
            console.log(`Template '${templateType}' not found in Firestore, using default template`);
        }
    } catch (error) {
    ...
    template = template.replace(/{customerName}/g, user.name);
    ...
    if (data) {
        ...
        if (data.amount) {
            ...
            template = template.replace(/{invoiceAmount}/g, formatAmount(data.amount));
        }
        ...
        template = template.replace(/{billingPeriod}/g, billingPeriod);
        ...
        template = template.replace(/{invoiceId}/g, data.formattedId || data.id || '');
        
        // Handle message placeholders - both {message} and {alertMessage}
        if (data.message) {
...
async function sendInvoicePdf(user, invoice, imageBlob, caption) {
    try {
        ...
        if (response.ok) {
            ...
            // Fallback to sending the full message template if image fails
            console.log("Falling back to sending full message template.");
            
            // If we already have a caption (which should be the full message), use that
            if (caption) {
                await sendMessage(user.phone, caption);
            } else {
                // Otherwise, generate the payment confirmation message
                const message = MESSAGE_TEMPLATES.PAYMENT_CONFIRMATION
                    .replace(/{userName}/g, user.name)
                    .replace(/{formattedId}/g, invoice.formattedId || invoice.id)
                    .replace(/{packageName}/g, invoice.packageName || '')
                    .replace(/{packageSpeed}/g, invoice.packageSpeed || '')
                    .replace(/{invoiceAmount}/g, formatAmount(invoice.amount))
                    .replace(/{invoiceMonth}/g, new Date(invoice.date).toLocaleString('default', { month: 'long' }))
                    .replace(/{invoiceYear}/g, new Date(invoice.date).getFullYear())
                    .replace(/{invoiceLink}/g, generateInvoiceLink(invoice.id));
...
async function sendBroadcastAlert(users, message) {
    // Calculate some stats for the return value
    let sent = 0;
    let failed = 0;
    
    const result = await sendBatchMessages(users, 'BROADCAST_ALERT', { message });
    
    // Count successes and failures
    if (result.success) sent += result.success.length;
    if (result.failed) failed += result.failed.length;
